...
public class PlayerController : MonoBehaviourPun
{
    ...
    [PunRPC]
    public void GodMode(int timeActivated)
    {
        if (godMode)
            return;

        Debug.Log("God Mode Activated for " + timeActivated + " seconds");
        ...
        IEnumerator GodModeCoRoutine()
        {
            ...
            halo.enabled = true;
            GameUI.instance.AddGodMode();
            moveSpeed *= 2;
            jumpForce *= 2;
            photonView.RPC("Heal", photonPlayer, maxHp);

            yield return new WaitForSeconds(timeActivated);

            GameUI.instance.DeactivateGodMode();
            moveSpeed /= 2;
            jumpForce /= 2;
            halo.enabled = false;
            godMode = false;
